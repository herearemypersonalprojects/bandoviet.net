...
public class AccentRemover {
  ...
  private static char[] REPLACEMENTS = { '-', '\0', '\0', '\0', '\0', '\0', '\0', '\0',
  ...
  public static String toUrlFriendly(String s) {
    ...
    String titleInUrl = result.trim().replaceAll("[^a-zA-Z0-9\\-\\s\\.]", "");
    titleInUrl = titleInUrl.replaceAll("[\\-| |\\.]+", "-");
    if (titleInUrl.length() > TITLE_IN_URL_MAX_LENGTH){
      result = titleInUrl.substring(0, TITLE_IN_URL_MAX_LENGTH);
    } else {
      result = titleInUrl;
    }
    return result;
